...
hdlr = logging.FileHandler('/home/roman/i2nsrec.log')
formatter = logging.Formatter('%(asctime)s %(levelname)s %(message)s')
hdlr.setFormatter(formatter)
logger.addHandler(hdlr) 
logger.setLevel(logging.INFO)

dira = "/mnt/d/iTunes/iTunes Media/Music/"
...
def getFirstFileWithSongNameContaining(dir, songName, extension):
	logger.info("looking for "+ extension + " file for song '" + songName + "' in '" + dir + "'")
...
def processitunesdir(dirName, parentDir):
	...
	if not dbPath:
		...
		dbPath = os.path.join(dirb, path[33:])
		logger.info("new dropbox path: " + dbPath)
		logger.warning("no dropbox dir found for " + path + ".  Creating new directory at " + dbPath)
		os.makedirs(dbPath)
	mp3Dir = os.path.join(dbPath, "mp3")
	if hasSubDirNamed(dbPath, "mp3"):
		if not os.listdir(mp3Dir):
			logger.warning("found empty mp3 dir in " + dbPath + "...deleting it")
			os.rmdir(mp3Dir)
		else: 
			logger.warning(dbPath + " has a non-empty mp3 directory, so it appears to have already been processed.  Skipping.")
	else: 
		if hasSubDirNamed(dbPath, "aac"):
			logger.warning(dbPath + " has an aac directory, so it appears to have already been processed.  Skipping.")   
		else:
			...
			if fileCount(path, ".m4a") > fileCount(dbPath, ".m4a"):
				...
				logger.warning("not enough aac files in " + dbPath + " to warrant move.  Skipping")
